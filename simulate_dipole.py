...
lense1 = PlanConvex(x=0.0, y=0,r=2.0, num=num)
lense2 = PlanConvex(x=0.0, y=0,r=2.0, num=num)

lense1.shift(dx=lense1.f)
lense1.flip()

lense2.shift(dx=lense1.x+lense2.f*2)
...
dipole = make_dipole(theta, num)
print("pointsource: " + str(dipole.n))
...
if plotit:
    ...
    for i in range(dipole.r.shape[0]):
        ...
        plt.arrow(dipole.r[i, 0], dipole.r[i, 1], dipole.k[i, 0] / divider, dipole.k[i, 1] / divider)
    plt.plot(lense1.front.points[:, 0], lense1.front.points[:, 1])
